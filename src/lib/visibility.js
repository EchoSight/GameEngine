...
import { getObstacleSegments } from './obstacles.js';
...
export function isBlocked(ox, oy, tx, ty, obstacles) {
...
export function isVisible(px, py, viewers, obstacles) {
    for (const v of viewers) {
        const dist = Math.sqrt((px - v.x) ** 2 + (py - v.y) ** 2);
        if (dist > v.visionRadius)
            continue;
        if (!isBlocked(v.x, v.y, px, py, obstacles))
            return true;
    }
    return false;
}
/** Check if a grid cell center is visible to any viewer */
export function isCellVisible(col, row, gridSize, viewers, obstacles) {
...
export function isMovementBlocked(ox, oy, tx, ty, obstacles) {
    for (const obs of obstacles) {
